...
export class UpdateRestaurantSubacc1761808463869 implements MigrationInterface {
    ...
    await queryRunner.addColumns('restaurant_subaccounts', [
      new TableColumn({ name: 'account_name', type: 'varchar', length: '255', isNullable: true }),
      new TableColumn({ name: 'account_number', type: 'varchar', length: '64', isNullable: true }),
      new TableColumn({ name: 'bank_code', type: 'varchar', length: '50', isNullable: true }),
    ]);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    const existingIndex = table?.indices.find(i => i.name === 'idx_restaurant_subaccounts_account_number');
    if (existingIndex) {
    ...
    for (const col of columnsToDrop) {
      const hasColumn = (await queryRunner.getTable('restaurant_subaccounts'))?.findColumnByName(col);
      if (hasColumn) {
