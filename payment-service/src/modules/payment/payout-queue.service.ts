...
export class PayoutQueueService implements OnModuleDestroy {
  ...
  constructor() {
    ...
    this.queue = new QueueCtor('payout-queue', {
  ...
  async addProcessBatchJob(
    payload: { batchId: string; requestedBy?: string; requestId?: string },
    ...
    this.logger.log(
      `Enqueued process-batch job id=${job.id} batchId=${payload.batchId} attempts=${(job.opts as any)?.attempts}`,
    );
    return job;
  }
  ...
  async onModuleDestroy() {
    ...
    try {
      ...
      this.logger.warn(
        'Error quitting redis (queue service)',
        (e as any)?.message ?? e,
      );
    }
  }
