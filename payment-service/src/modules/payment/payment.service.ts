...
async createAggregatedBatch(opts: {
  ...
  const minTotal = typeof opts.minTotal === 'number' ? Number(opts.minTotal) : null;
  ...
  return await this.dataSource.transaction(async (manager) => {
    ...
    const batchEntity = batchRepo.create({
      status: 'created',
      total_amount: 0,
      meta: { created_by: createdBy, created_at: new Date().toISOString(), autoProcess: !!opts.autoProcess },
    } as any);
    const savedBatchRes = await batchRepo.save(batchEntity);
    const savedBatch: PayoutBatch = Array.isArray(savedBatchRes) ? savedBatchRes[0] : savedBatchRes;
    ...
    if (opts.olderThan) {
      olderThanDate = opts.olderThan instanceof Date ? opts.olderThan : new Date(String(opts.olderThan));
    ...
    if (olderThanDate) {
      qb.andWhere('c.created_at <= :olderThan', { olderThan: olderThanDate.toISOString() });
    }
    if (opts.restaurantIds && Array.isArray(opts.restaurantIds) && opts.restaurantIds.length > 0) {
      qb.andWhere('c.restaurant_id IN (:...rids)', { rids: opts.restaurantIds });
    }
    ...
    for (const r of rows) {
      ...
      if (minTotal !== null && t < minTotal) {
        details.skipped.push({ restaurant_id: r.restaurant_id, reason: 'below_min_total', total: t });
        continue;
      }
      ...
      if (!sub) {
        details.skipped.push({ restaurant_id: r.restaurant_id, reason: 'missing_subaccount', total: t });
        continue;
      }
      if (!sub.account_number || !sub.account_name || !sub.bank_code) {
        details.skipped.push({ restaurant_id: r.restaurant_id, reason: 'missing_bank_details', missing: { account_number: !!sub.account_number, account_name: !!sub.account_name, bank_code: !!sub.bank_code }, total: t });
        continue;
      }
      ...
      const aggEntity = aggRepo.create({
        ...
        meta: { child_ids: r.child_ids, created_by: createdBy, created_at: new Date().toISOString() },
      } as any);
      ...
      totalForBatch += t;
      details.created_aggregates.push({ aggregate_id: savedAgg.id, restaurant_id: r.restaurant_id, total: t, child_count: r.child_ids.length });
    }

    // finalize batch totals and save
    savedBatch.total_amount = Number(Math.round((totalForBatch + Number.EPSILON) * 100) / 100);
    const finalBatchRes = await batchRepo.save(savedBatch);
    const finalBatch: PayoutBatch = Array.isArray(finalBatchRes) ? finalBatchRes[0] : finalBatchRes;
...
async processAggregatedBatch(batchId: string, p0: { requestedBy: any; force: boolean }) {
  ...
  const maxStaleMinutes = Number(process.env.PAYOUT_PROCESSING_STALE_MINUTES ?? 15);
  const staleCutoff = new Date(Date.now() - maxStaleMinutes * 60_000).toISOString();
  ...
  const claimedRow = (updateRes && (updateRes as any).raw && (updateRes as any).raw[0]) ?? null;
  ...
  if (!claimedRow) {
    ...
    if (existing.status === 'processing') {
      ...
      const summary = await aggRepo
        ...
        .select("COUNT(*) FILTER (WHERE a.status = 'processing')", 'processing_count')
        .addSelect("COUNT(*) FILTER (WHERE a.status = 'batched')", 'batched_count')
        .addSelect("COUNT(*) FILTER (WHERE a.status = 'failed')", 'failed_count')
      ...
      if (processingCount === 0 && batchedCount === 0) {
        ...
        existing.meta = { ...(existing.meta ?? {}), finalized_by: requestedBy, finalized_at: new Date().toISOString() };

        await batchRepo.save(existing);
        logger.log(`Batch ${batchId} had no active aggregates — finalized as ${existing.status} (paid=${paidCount} failed=${failedCount})`);
        return existing;
      }

      // Otherwise it's actively processing by someone else - no-op
      logger.warn(`Batch ${batchId} is already processing by another worker — exiting job (no-op).`);
      return existing;
    }
  ...
  try {
    providerResp = await this.chapa.bulkTransfer({ transfers, title: `Payout Batch ${batchId}`, currency: 'ETB', reference: batchId });
  } catch (err) {
  ...
  batch.meta = { ...(batch.meta ?? {}), provider_response: providerResp?.raw ?? providerResp };
  ...
  for (const a of aggs) {
    ...
    if (!res) {
      ...
      await aggRepo.update({ id: a.id }, { provider_response: (a.provider_response ?? null) || { queued: true } } as any);
      continue;
    }
    ...
    if (['success', 'paid', 'completed', 'ok'].includes(normalizedStatus)) {
      await aggRepo.update({ id: a.id }, { status: 'paid', provider_transfer_id: res.transfer_id ?? res.id ?? null, provider_response: res.raw ?? res } as any);
      ...
      if (childIds && childIds.length) {
        await childRepo
          ...
          .set({
            status: 'paid',
            meta: () => `jsonb_set(coalesce(meta,'{}'::jsonb), '{paid_via}', to_jsonb('${String(res.transfer_id ?? res.id ?? '')}'))`,
          })
      ...
      try {
        await this.kafka.emit('payout.item.paid', { payout_item_id: a.id, restaurant_id: a.restaurant_id, amount: a.amount, provider_transfer_id: res.transfer_id ?? res.id ?? null });
      } catch (e) {
        logger.warn('Failed to emit payout.item.paid', e?.message ?? e);
      }
    } else {
      const lastErr = res.error ?? res.raw?.error_message ?? res.raw?.message ?? 'provider_failed';
      await aggRepo.update({ id: a.id }, { status: 'failed', last_error: lastErr, provider_response: res.raw ?? res } as any);
      ...
      if (childIds && childIds.length) {
        await childRepo
          ...
          .set({
            status: 'failed',
            meta: () => `jsonb_set(coalesce(meta,'{}'::jsonb), '{last_error}', to_jsonb('${String(lastErr).replace(/'/g, "''")}'))`,
          })
          .where('id IN (:...ids)', { ids: childIds })
          .execute();
      }
      ...
      try {
        await this.kafka.emit('payout.item.failed', { payout_item_id: a.id, restaurant_id: a.restaurant_id, amount: a.amount, error: lastErr });
      } catch (e) {
