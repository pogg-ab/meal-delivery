...
async bulkTransfer(opts: {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      const res = await this.client.post('/v1/bulk-transfers', payload, { headers });
      ...
      if (Array.isArray(rawItems)) {
        ...
        for (const it of rawItems) {
          const referenceField = it?.reference ?? it?.ref ?? it?.payout_reference ?? null;
          const payout_item_id = referenceField ? String(referenceField) : null;

          const transfer_id = it?.transfer_id ?? it?.id ?? it?.transaction_id ?? it?.txn_id ?? null;
          const status = (it?.status ?? it?.state ?? it?.result ?? null) ? String(it?.status ?? it?.state ?? it?.result).toLowerCase() : null;
      ...
      logger.warn('Chapa bulkTransfer attempt failed', { attempt, err: err?.response?.data ?? err?.message ?? err });
      if (attempt === 2) {
