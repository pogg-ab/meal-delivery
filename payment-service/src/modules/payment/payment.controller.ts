...
export class PaymentsController {
  ...
  @ApiOperation({
    summary:
      'Chapa webhook endpoint (server-to-server). Provide raw JSON body and X-Chapa-Signature header.',
  })
  @ApiHeader({
  ...
  @ApiOperation({
    summary:
      'Create chapa subaccount for a restaurant (internal only). Provide restaurant id as path param.',
  })
  ...
  async createSub(
    ...
    if (secret !== process.env.SERVICE_AUTH_SECRET)
  ...
  @ApiBearerAuth('access-token')
  @ApiOperation({
    summary: 'Create or update the platform chapa subaccount (internal only)',
  })
  @ApiBody({ type: CreateSubaccountDto })
  ...
  @Roles('platform_admin')
  @ApiOperation({
    summary: 'List restaurant subaccounts (supports pagination and filtering)',
  })
  @ApiOkResponse({
    description: 'Returns a paginated list of restaurant subaccounts',
  })
  ...
  @ApiBearerAuth('access-token')
  @ApiOperation({
    summary: 'Create aggregated batch (admin). Optionally auto-process.',
  })
  async createAggregatedBatch(
    @Req() req: any,
    @Body() dto: CreateAggregatedBatchDto,
  ) {
    ...
    if (dto.autoProcess) {
      const job = await this.payoutQueue.addProcessBatchJob({
        batchId: batch.id,
  ...
  @ApiBearerAuth('access-token')
  @ApiOperation({
    summary: 'Trigger processing of an existing batch (enqueue or sync)',
  })
  async processBatch(
    @Req() req: any,
    @Param('id') id: string,
    ...
    if (dto?.sync) {
      ...
      const result = await this.svc.processAggregatedBatch(id, {
        requestedBy,
