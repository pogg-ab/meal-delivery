...
export class PromoCodeService {
  ...
  async deletePromos(dto: { promoId?: string; restaurantId?: string }): Promise<{ deleted: number }> {
    const { promoId, restaurantId } = dto;
    this.logger.log(`Deleting promos with promoId=${promoId} restaurantId=${restaurantId}`);
      try {
    let res;
     if (promoId) {
     res = await this.promoRepo.delete({ id: promoId });
    } else if (restaurantId) {
     res = await this.promoRepo.delete({ applicable_restaurant_id: restaurantId });
   } else {
    res = await this.promoRepo.delete({ issuer_type: 'platform' });
    }  
   const deleted = typeof res.affected === 'number' ? res.affected : 0;
   this.logger.log(`Deleted ${deleted} promo code(s)`);
   return { deleted };
    } catch (err: any) {
    this.logger.error('Failed deleting promo codes', err?.message ?? err);
   throw err;
   }
  }
}
